...
class MultiProviderAI {
    ...
    // Cache & throttle
    cache = new Map();
    CACHE_TTL = 120_000; // 2 min
    BLACKLIST_BASE = 120_000; // 2 min base blacklist
    lastRequestTime = 0;
    MIN_INTERVAL = 3_000; // 3 s between provider calls
    stateFile = path.join(__dirname, '../../data/ai-state.json');
    constructor() {
        this.loadKeys();
        this.loadState();
    }
    ...
    async loadState() {
        try {
            ...
            const now = Date.now();
            const restore = (keys, saved) => {
                keys.forEach((k, i) => {
                    if (saved?.[i]) {
                        if (saved[i].blacklistedUntil > now)
                            k.blacklistedUntil = saved[i].blacklistedUntil;
    ...
    async *tryOpenRouterStream(prompt) {
        const available = this.getAvailable(this.openRouterKeys);
        if (!available.length)
            return;
        for (const key of this.rotate(available)) {
     ...
     */
    async generate(prompt, context = '', _userId) {
     ...
     */
    async *generateStream(prompt, context = '', _userId) {
        const fullPrompt = context ? `[User context: ${context}]\n\nUser: ${prompt}` : prompt;
        // 1. Gemini stream
    ...
    // ── Feature Helpers ─────────────────────────────────────────────────────────
    async generateDashboardSummary(ctx) {
        const phasePrompts = {
    ...
    async generateAskAIResponse(userContext, query) {
        const r = await this.generate(query, JSON.stringify(userContext));
        return r.text;
    }
    async generateMealSuggestions(userContext, meal) {
        const r = await this.generate(`User logged "${meal.description}" (${meal.mealType}). Give one cycle-phase nutritional tip.`, JSON.stringify(userContext));
        return r.text;
    }
    async generateSleepSuggestions(userContext, sleep) {
        const r = await this.generate(`User slept ${sleep.hours}h, quality "${sleep.quality}". Give one practical sleep tip.`, JSON.stringify(userContext));
        return r.text;
    }
    async generateCravingAlternatives(cravings, phase) {
        const r = await this.generate(`Cravings: [${cravings.join(', ')}] during ${phase}. Give 3 healthy alternatives and one tip. JSON only: {"alternatives":["a","b","c"],"advice":"tip"}`, '');
        try {
    ...
    getFallbackResponse(prompt, _context = '') {
