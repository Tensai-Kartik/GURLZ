...
router.get('/dashboard/summary', authenticateToken, async (req, res) => {
    try {
        ...
        const hydrationAmount = hydrationLogs.reduce((acc, log) => acc + log.amountMl, 0);
        const hydrationGoal = hydrationLogs[0]?.goalMl || 2000;
        let score = 70;
        if (hydrationAmount >= hydrationGoal)
            score += 10;
        else if (hydrationAmount >= hydrationGoal * 0.5)
            score += 5;
        if (meals.length >= 3)
            score += 10;
        else if (meals.length >= 1)
            score += 5;
        if (latestSleep && Number(latestSleep.hours) >= 7)
            score += 10;
