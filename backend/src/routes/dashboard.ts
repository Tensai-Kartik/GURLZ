...
// Server-side summary cache: prevents hammering Gemini on every dashboard load
const summaryCache = new Map<string, { summary: string; ts: number }>();
const SUMMARY_CACHE_TTL = 10 * 60 * 1000; // 10 minutes per user
...
export async function dashboardRoutes(fastify: FastifyInstance) {
  fastify.get('/dashboard/summary', {
    ...
    try {
      ...
      // Check server-side cache before calling Gemini (saves quota)
      ...
      return reply.code(500).send({ error: 'Failed to build dashboard summary' });
    }
  });
}
