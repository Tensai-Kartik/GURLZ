...
router.put('/settings', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    let currentSettings = {};
    if (user?.settings) {
      try { currentSettings = JSON.parse(user.settings); } catch {}
    }

    const updatedSettings = JSON.stringify({
      ...currentSettings,
      theme: data.theme || (currentSettings as any).theme || 'pink-soft',
    });
...
router.delete('/settings/data', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
